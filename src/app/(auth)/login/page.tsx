"use client";

import React, { useState, useEffect, Suspense } from 'react';
import { useRouter, useSearchParams } from 'next/navigation';
import { Button } from '@/components/ui/Button';
import { Input } from '@/components/ui/Input';
import { useAuth } from '@/hooks/useAuth';
import { useToast } from '@/components/ui/Toast';
import Image from 'next/image';

type LoginTab = 'google' | 'student';

function LoginContent() {
  const [tab, setTab] = useState<LoginTab>('google');
  const [isLoading, setIsLoading] = useState(false);
  const [studentIdInput, setStudentIdInput] = useState('');
  const router = useRouter();
  const searchParams = useSearchParams();
  const { signInWithGoogle, signInWithStudentId, user, userDoc, role } = useAuth();
  const { showToast } = useToast();

  useEffect(() => {
    if (user && userDoc) {
      // Original mode students are pre-approved with complete profiles
      if (userDoc.loginType === 'original') {
        const redirect = searchParams?.get('redirect');
        router.push(redirect || '/student');
        return;
      }
      const complete = userDoc.name && userDoc.classRoom && userDoc.studentId;
      if (!complete) { router.push('/complete-profile'); return; }
      if (userDoc.status === 'pending'  && role !== 'admin') { router.push('/pending-approval'); return; }
      if (userDoc.status === 'rejected' && role !== 'admin') { router.push('/rejected'); return; }
      const redirect = searchParams?.get('redirect');
      router.push(redirect || (role === 'admin' ? '/admin' : '/student'));
    }
  }, [user, userDoc, role, router, searchParams]);

  const handleGoogleLogin = async () => {
    setIsLoading(true);
    try {
      await signInWithGoogle();
    } catch {
      showToast('เกิดข้อผิดพลาดในการเข้าสู่ระบบ', 'error');
      setIsLoading(false);
    }
  };

  const handleStudentIdLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!studentIdInput.trim()) {
      showToast('กรุณากรอกเลขประจำตัวนักเรียน', 'error');
      return;
    }
    setIsLoading(true);
    try {
      await signInWithStudentId(studentIdInput.trim());
    } catch (err: any) {
      const code = err?.code ?? '';
      if (code === 'functions/not-found') {
        showToast('ไม่พบเลขประจำตัวนักเรียนในระบบ', 'error');
      } else {
        showToast('เกิดข้อผิดพลาดในการเข้าสู่ระบบ', 'error');
      }
      setIsLoading(false);
    }
  };

  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden page-transition">

      {/* Blurred glow orbs */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="login-blob-1" />
        <div className="login-blob-2" />
        <div className="login-blob-3" />
      </div>

      {/* Star field */}
      <div className="star-field" />

      {/* Grid overlay */}
      <div className="login-grid" />

      {/* Login card */}
      <div className="relative w-full max-w-[380px] z-10">
        <div className="holo-border rounded-[20px]">
          <div className="
            relative
            bg-[rgba(255,255,255,0.065)] backdrop-blur-[40px]
            rounded-[20px] p-8 text-center
            border border-[rgba(255,255,255,0.10)]
            shadow-[0_24px_72px_rgba(0,0,0,0.55)]
          ">
            {/* Corner brackets */}
            <span className="absolute top-4 left-4 w-5 h-5 border-t-2 border-l-2 border-[rgba(99,102,241,0.40)] rounded-tl-sm" />
            <span className="absolute top-4 right-4 w-5 h-5 border-t-2 border-r-2 border-[rgba(99,102,241,0.40)] rounded-tr-sm" />
            <span className="absolute bottom-4 left-4 w-5 h-5 border-b-2 border-l-2 border-[rgba(99,102,241,0.40)] rounded-bl-sm" />
            <span className="absolute bottom-4 right-4 w-5 h-5 border-b-2 border-r-2 border-[rgba(99,102,241,0.40)] rounded-br-sm" />

            {/* Logo + Brand */}
            <div className="mb-7 flex flex-col items-center gap-4">
              <div className="float-anim drop-shadow-[0_8px_32px_rgba(99,102,241,0.50)]">
                <Image
                  src="/logo.png"
                  alt="STEMFOLIO"
                  width={180}
                  height={180}
                  priority
                  className="rounded-full"
                />
              </div>
              <div>
                <p className="text-[12px] font-semibold text-white/55 uppercase tracking-[0.22em]">
                  Projects Management System
                </p>
              </div>
            </div>

            {/* Status indicator */}
            <div className="flex items-center justify-center gap-2 mb-6">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.65)]" />
              <span className="text-[10px] font-semibold text-white/55 tracking-widest uppercase">
                Auth Gateway Online
              </span>
            </div>

            {/* Tab toggle */}
            <div className="flex rounded-[var(--radius-input)] bg-white/[0.06] p-1 mb-6 gap-1">
              <button
                type="button"
                onClick={() => { setTab('google'); setStudentIdInput(''); }}
                className={`flex-1 py-2 text-xs font-semibold rounded-[10px] transition-all ${
                  tab === 'google'
                    ? 'bg-white/15 text-white shadow-sm'
                    : 'text-white/45 hover:text-white/70'
                }`}
              >
                Google
              </button>
              <button
                type="button"
                onClick={() => { setTab('student'); }}
                className={`flex-1 py-2 text-xs font-semibold rounded-[10px] transition-all ${
                  tab === 'student'
                    ? 'bg-white/15 text-white shadow-sm'
                    : 'text-white/45 hover:text-white/70'
                }`}
              >
                เลขประจำตัว
              </button>
            </div>

            {tab === 'google' ? (
              <>
                <Button
                  type="button"
                  variant="secondary"
                  onClick={handleGoogleLogin}
                  className="w-full h-11 text-sm font-semibold gap-3 !justify-center"
                  loading={isLoading}
                >
                  {!isLoading && (
                    <svg viewBox="0 0 24 24" width="17" height="17" xmlns="http://www.w3.org/2000/svg">
                      <g transform="matrix(1,0,0,1,27.009,-39.239)">
                        <path fill="#4285F4" d="M -3.264 51.509 C -3.264 50.719 -3.334 49.969 -3.454 49.239 L -14.754 49.239 L -14.754 53.749 L -8.284 53.749 C -8.574 55.229 -9.424 56.479 -10.684 57.329 L -10.684 60.329 L -6.824 60.329 C -4.564 58.239 -3.264 55.159 -3.264 51.509 Z"/>
                        <path fill="#34A853" d="M -14.754 63.239 C -11.514 63.239 -8.804 62.159 -6.824 60.329 L -10.684 57.329 C -11.764 58.049 -13.134 58.489 -14.754 58.489 C -17.884 58.489 -20.534 56.379 -21.484 53.529 L -25.464 53.529 L -25.464 56.619 C -23.494 60.539 -19.444 63.239 -14.754 63.239 Z"/>
                        <path fill="#FBBC05" d="M -21.484 53.529 C -21.734 52.809 -21.864 52.039 -21.864 51.239 C -21.864 50.439 -21.724 49.669 -21.484 48.949 L -21.484 45.859 L -25.464 45.859 C -26.284 47.479 -26.754 49.299 -26.754 51.239 C -26.754 53.179 -26.284 54.999 -25.464 56.619 L -21.484 53.529 Z"/>
                        <path fill="#EA4335" d="M -14.754 43.989 C -12.984 43.989 -11.404 44.599 -10.154 45.789 L -6.734 42.369 C -8.804 40.429 -11.514 39.239 -14.754 39.239 C -19.444 39.239 -23.494 41.939 -25.464 45.859 L -21.484 48.949 C -20.534 46.099 -17.884 43.989 -14.754 43.989 Z"/>
                      </g>
                    </svg>
                  )}
                  {isLoading ? 'กำลังเข้าสู่ระบบ…' : 'เข้าสู่ระบบด้วย Google'}
                </Button>
                <p className="mt-5 text-[15px] font-medium text-white/45 leading-relaxed">
                  เฉพาะนักเรียนและครูที่ได้รับอนุญาตเท่านั้น
                </p>
              </>
            ) : (
              <form onSubmit={handleStudentIdLogin} className="flex flex-col gap-3 text-left">
                <Input
                  label="เลขประจำตัวนักเรียน"
                  placeholder="เช่น 12345"
                  value={studentIdInput}
                  onChange={e => setStudentIdInput(e.target.value)}
                  disabled={isLoading}
                  autoComplete="off"
                  autoFocus
                />
                <Button
                  type="submit"
                  className="w-full h-11 text-sm font-semibold mt-1"
                  loading={isLoading}
                >
                  {isLoading ? 'กำลังเข้าสู่ระบบ…' : 'เข้าสู่ระบบ'}
                </Button>
                <p className="text-[11px] text-white/35 text-center leading-relaxed mt-1">
                  สำหรับนักเรียนที่ได้รับการลงทะเบียนจากครูเท่านั้น
                </p>
              </form>
            )}
          </div>
        </div>
      </div>
    </div>
  );
}

export default function LoginPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center">
        <span className="cyber-spinner w-8 h-8" />
      </div>
    }>
      <LoginContent />
    </Suspense>
  );
}
